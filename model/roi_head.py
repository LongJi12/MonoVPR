...
from .GDA import GatedDualPathAttention
...
class IterativeOffsetRefinement(nn.Module):
    def __init__(self, input_dim, hidden_dim=256, iterations=3):
        super().__init__()
        ...
        # 共享单个 refine block
        self.refine_block = RefineBlock(input_dim + 3)
        
        # 初始偏移预测
        self.init_offset = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.LayerNorm(hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, 3)
    ...
    def forward(self, roi_feats):
        ...
        # 预先扩展 ROI 特征
        expanded_roi = roi_feats.unsqueeze(1).expand(-1, 1352, -1)  # (N, 1352, C)
        
        # 初始预测并广播到所有顶点
        base_offsets = self.init_offset(roi_feats).unsqueeze(1).repeat(1, 1352, 1)
        
        current_offsets = base_offsets.to(roi_feats.device)  # 确保设备一致
        
        for _ in range(self.iterations):
            # 动态拼接输入特征和当前偏移量
            combined = torch.cat([expanded_roi, current_offsets], dim=-1)
            delta = self.refine_block(combined)
            
            # 使用 in-place 更新
            current_offsets.add_(0.5 * torch.tanh(delta))
...
class MultiHeadShapeAttention(nn.Module):
    def __init__(self, input_dim, num_head=8, dropout=0.1):
        super().__init__()
        self.num_head = num_head
        self.head_dim = input_dim // num_head
        assert self.head_dim * num_head == input_dim, "input_dim must be divisible by num_head"
        ...
        # 形状基参数
        self.E = nn.Parameter(torch.randn(79, input_dim))
    ...
    def forward(self, x):
        ...
        # 投影形状基参数
        k = self.k_proj(self.E).view(1,79, self.num_head, self.head_dim)  # (79, H, D)
        v = self.v_proj(self.E).view(1,79, self.num_head, self.head_dim)  # (79, H, D)
        
        # 查询向量
        q = self.q_proj(x).view(batch_size, 1, self.num_head, self.head_dim)  # (N, 1, H, D)
        
        # 计算注意力 - 修正einsum表达式
        # 将q和k转换为兼容的形状
        q = q.permute(0, 2, 1, 3)  # (N, H, 1, D)
        k = k.permute(0, 2, 1, 3)  # (1, H, 79, D)
        
        # 计算注意力分数
        attn_scores = torch.matmul(q, k.transpose(-1, -2)) / math.sqrt(self.head_dim)  # (N, H, 1, 79)
        attn_weights = F.softmax(attn_scores, dim=-1)  # (N, H, 1, 79)
        attn_weights = self.dropout(attn_weights)
        
        # 准备v用于加权求和
        v = v.permute(0, 2, 1, 3)  # (1, H, 79, D)
        
        # 加权求和
        weighted_v = torch.matmul(attn_weights, v)  # (N, H, 1, D)
        weighted_v = weighted_v.permute(0, 2, 1, 3).contiguous().view(batch_size, -1)  # (N, H*D)
        
        # 输出变换
        output = self.out_proj(weighted_v)
        return self.norm(x + output)


class ShapeFusion(nn.Module):
    def __init__(self, M, V, input_dim, num_head=8, attention_layers=2):
        super().__init__()
        self.n_verts = M.shape[1] // 3
        self.register_buffer('M_template', M.view(1, self.n_verts, 3))
        
        # 多头注意力层堆叠
        self.attention_layers = nn.ModuleList([
            MultiHeadShapeAttention(input_dim, num_head)
            for _ in range(attention_layers)
        ])
        
        # 形状基参数
        self.V = nn.Parameter(V, requires_grad=False)  # (79, 4056)
        self.shape_proj = nn.Linear(input_dim, 79)
        
        # 迭代修正模块
        self.refinement = IterativeOffsetRefinement(input_dim)
    ...
    def forward(self, x):
        ...
        # 生成形状基权重
        shape_weights = F.softmax(self.shape_proj(x), dim=-1)  # (N, 79)
        
        # 计算基础形状
        base_verts = self.M_template + (shape_weights @ self.V).view(-1, self.n_verts, 3)
        
        # 迭代细化
        refined_offsets = self.refinement(x)
        
        # 融合结果
        final_verts = (
            self.fusion_weight[0] * base_verts +
            self.fusion_weight[1] * refined_offsets
        )
        return final_verts
...
class MonoVPRROIHeads(ROIHeads):
    """
    ...
    def _init_pose_head(self, input_dim):
        ...
        self._shape_fusion = ShapeFusion(self.init_M, self.init_V, self.hidden_dim,num_head = 8, attention_layers=2)
        self.gda = GatedDualPathAttention(288).to(self.device)
    ...
    def _forward_3d_pose(self, features, proposals):
        ...
        fused_features = F.interpolate(features['p3'], size=target_size, mode='bilinear', align_corners=False)
        p4_downsampled = F.interpolate(features['p4'], size=target_size, mode='bilinear', align_corners=False)
        p5_downsampled = F.interpolate(features['p5'], size=target_size, mode='bilinear', align_corners=False)
        p6_downsampled = F.interpolate(features['p6'], size=target_size, mode='bilinear', align_corners=False)
        p7_downsampled = features['p7'] 

       
        all_features = torch.cat([fused_features, p4_downsampled, p5_downsampled, p6_downsampled, p7_downsampled], dim=1)
        
        self.conv_fusion = nn.Conv2d(all_features.shape[1], 288, kernel_size=3, padding=1).to(self.device)
        features_before_gda = self.conv_fusion(all_features)

       
        features_after_gda = self.gda(features_before_gda)

        global_features = self._global_features(features_after_gda) 
        ...
        if not self.training:
            starter_mesh = torch.cuda.Event(enable_timing=True)
            ender_mesh = torch.cuda.Event(enable_timing=True)
            starter_mesh.record()
