...
class Detect3DEval(object):
    ...
    for g in gt:
        g['area'] = _get_area(g['bbox'])
        g['area_label'] = next(i for i, (a_min, a_max) in enumerate(self.areaRng)
                            if a_min <= g['area'] < a_max)

    for d in dt:
        d['area'] = _get_area(d['bbox'])
        d['area_label'] = next(i for i, (a_min, a_max) in enumerate(self.areaRng)
                            if a_min <= d['area'] < a_max)
